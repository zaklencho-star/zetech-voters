...
# -------------------------------------------------------- unified gateway --
# One login box. One page. It figures out whether you're a student or the
# returning officer from what you typed — no separate "voter vs admin" split.
...
def home():
    ...
    if request.method == "POST":
        credential_input = request.form.get("credential", "")
        raw = credential_input.strip()

        conn = get_db()
        try:
            # 1. Does it match the returning officer's passcode?
            admin_user = conn.execute(
                "SELECT * FROM admin_users WHERE username = ?", (DEFAULT_ADMIN_USERNAME,)
            ).fetchone()
            if admin_user and raw and check_password_hash(admin_user["password_hash"], raw):
                session.clear()
                session["is_admin"] = True
                session["admin_username"] = admin_user["username"]
                return redirect(url_for("admin_candidates"))

            # 2. Otherwise, treat it as a student admission number.
            admission = normalize_admission(raw)
            if not is_valid_admission(admission):
                error = "That's not a recognised admission number or the admin passcode. Use the format COURSECODE/NNNN/YY, e.g. BIRD/0124/25."
            else:
    ...
    return render_template("home.html", error=error, credential_input=credential_input)
